...
const SummarySection = ({ collection, result, loading, hideResult, onCollectionChange, onSubmit }) => {
    ...
    <div className="col gap12">
      <div className="sec-head" style={{ padding: 0, border: 'none', marginBottom: 4 }}>
        <h3 className="h3 row gap8">
          ...
          {t('ragPage.createSummary')}
        </h3>
      </div>
      <form onSubmit={onSubmit} className="col gap12">
        <div className="field">
          <label className="label" htmlFor="rag-sum-collection">{t('ragPage.collection')}</label>
          <input
            id="rag-sum-collection"
            className="input"
            type="text"
            value={collection}
